...
from django.urls import URLPattern, include
...
class TestSelectiveInclude(TestCase):
  ...
  def setUpClass(cls) -> None:
    ...
    cls.all_urls = list(
        cast(TypeIncludedModule,
             include(cls.module_name)[0]).urlpatterns
    )

  def _get_urlpatterns(self, module: TypeInclude) -> TypeUrlPatterns:
    return module[0].urlpatterns

  def _patterns_equal(
      self, urlpatterns1: TypeUrlPatterns, urlpatterns2: TypeUrlPatterns
  ) -> bool:
    for index, url1 in enumerate(urlpatterns1):
      url2 = urlpatterns2[index]
      if isinstance(url1, URLPattern) and isinstance(url2, URLPattern):
        if url1.name != url2.name:
          return False
    return True

  def _pattern_missing(self, patterns: TypeUrlPatterns, missing: str) -> bool:
    for url in patterns:
  ...
  def test_filter_1(self) -> None:
    result = selective_include(self.module_name, ["test1"])
    self.assertFalse(
  ...
  def test_filter_1_and_3(self) -> None:
    result = selective_include(self.module_name, ["test1", "test3"])
    self.assertFalse(
        self._patterns_equal(result[0].urlpatterns, self.all_urls),
    )
